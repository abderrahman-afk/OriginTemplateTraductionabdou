...
export class HorizontaltopbarComponent implements OnInit, AfterViewInit {
  ...
  constructor(@Inject(DOCUMENT) private document: any, private router: Router, private eventService: EventService, private authService: AuthenticationService,
    ...
    public languageService: LanguageService,private token:TokenStorage,
  ...
  initialize(): void {
    ...
    if(this.token.getUser().role_portail=="CHEF"){
      MENU[15].isLayout=false
    }else{
      MENU[15].isLayout=true
    ...
    if(this.token.getUser().role_portail=="ADMIN"){
      MENU[17].isAdmin=false
    }else{
      MENU[17].isAdmin=true
